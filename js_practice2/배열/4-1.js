// 4-1. 전달 받은 문자열(str) 안에 있는 값을 ',' 기준으로 배열로 분리하고 전달받은 
// 단어(word)가 있는 index 위치를 문자열로 리턴하는 함수 findWord(str, word)를 
// 작성한다.
...
function filterRange(arr, a, b) {
    ...
    for(let i = 0; i <arr.length; i++) {
        if (!(a <= arr[i] && arr[i] <= b)) { // 3이 1보다 크고 5보다 작지 않을 때 splice로 잘라낸다.
            arr.splice(i, 1);   // 요소 하나만 삭제
        }
    }
}
...
// 4-4. Student 생성자 함수를 통해 생성 된 3명의 학생 객체를 studentList 배열에 
// 담았다. 해당 배열을 전달하여 score 내림차순 기준으로 정렬해주는 sortFromScore(arr) 
// 함수, 해당 배열을 전달하여 lastName과 firstName을 합성하여 name 속성으로 만들어 
