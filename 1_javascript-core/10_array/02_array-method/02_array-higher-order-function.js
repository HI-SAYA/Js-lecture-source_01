...
// => 다른 정렬 기준을 사용하려면 compare 함수를 인수를 전달해야 함
numbers.sort();
...
numbers.sort((a, b) => b - a);      // 고차 함수의 콜백
...
/*
    배열.forEach(function(item, index, array){      // java 람다와 똑같음
        // 배열 요소 각각에 실행할 기능 작성
    }); 
*/
...
const max2 = [5, 1, 10, 8, 4].reduce((pre, cur) => pre > cur ? pre : cur);
...
result = [1, 5, 3, 2, 4].some(item => item > 4);
console.log(`result : ${result}`);
...
result = students.find(item => item.name == '손흥민');
console.log(result);
result = students.findIndex(item => item.name == '손흥민');
console.log(result);

// 만약 85점 이상인 학생들을 찾고 싶다면?
// find, findIndex는 일치하는 요소를 찾으면 더 이상 탐색하지 않고 하나의 요소, 인덱스만 반환한다.
// filter는 콜백 함수의 실행 결과가 true인 배열 요소의 값만을 추출한 새로운 배열을 반환한다.
result = students.filter(item => item.score >= 85);
console.log(result);